...
System::String^ NS_Comp_Mappage::CLmapStatistiques::SelectLessPopular(void) {
	return "SELECT ligne_commande.id_article, SUM(ligne_commande.qte_commandee) FROM ligne_commande FULL OUTER JOIN id_article ON ligne_commande.id_article = Article.id_article";
}
...
System::String^ NS_Comp_Mappage::CLmapStatistiques::SelectSousSeuilReap(void) {
	return "SELECT * FROM Article WHERE qte_stock < seuil_reapprovisionnement";
}
